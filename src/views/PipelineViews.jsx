    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <button onClick={() => { setEditing(null); setShowModal(true); }}
          className="flex items-center gap-2 px-4 py-2.5 bg-stone-900 text-white rounded-lg text-sm font-medium hover:bg-stone-800 transition">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {LEAD_STAGES.map((stage) => (
          <div key={stage.id}
            ...
            className="bg-stone-100 rounded-xl p-2 min-h-[300px]"
          >
            ...
            <div className="space-y-2">
              {grouped[stage.id].map((lead) => (
                <div key={lead.id}
                  ...
                  className="bg-white rounded-lg p-3 border border-stone-200 cursor-pointer hover:border-stone-900 transition">
                  ...
                  <div className="text-[10px] text-stone-600 mb-2 line-clamp-2">{lead.notes}</div>
              ...
              {grouped[stage.id].length === 0 && (
                <div className="text-xs text-stone-400 text-center py-6">Drop here</div>
              )}
            </div>
          </div>
        ))}
...
function LeadModal({ lead, onSave, onClose, onDelete }) {
  ...
  return (
    <Modal title={lead ? "Edit lead" : "New lead"} onClose={onClose}
      footer={<>
        {onDelete && <button onClick={onDelete} className="px-3 py-2.5 border border-rose-200 text-rose-700 rounded-lg text-sm font-medium hover:bg-rose-50">Delete</button>}
        <button onClick={onClose} className="flex-1 py-2.5 border border-stone-200 rounded-lg text-sm font-medium">Cancel</button>
        <button onClick={submit} className="flex-1 py-2.5 bg-stone-900 text-white rounded-lg text-sm font-medium">Save</button>
      </>}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Source</label>
            <select value={form.source} onChange={(e) => update("source", e.target.value)} className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm">
              {["Walk-in", "Instagram", "Facebook", "Referral", "Website", "Google Ads", "Other"].map((s) => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Interested in</label>
            <select value={form.interest} onChange={(e) => update("interest", e.target.value)} className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm">
...
export function AccessControlView({ accessLogs, setAccessLogs, members }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="font-display text-xl font-semibold mb-4">Doors</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {doors.map((d) => (
            <div key={d.id} className="bg-white rounded-xl border border-stone-200 p-5">
              ...
              <div className="text-xs font-mono text-stone-400 mb-4">Last activity: {d.lastActivity}</div>
              <button onClick={() => lockDoor(d.id)} className="w-full py-2 text-xs font-medium border border-stone-200 rounded-lg hover:border-stone-900">
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-5 sm:p-6">
        <h3 className="font-display text-xl font-semibold mb-4">Access log</h3>
        {accessLogs.length === 0 ? (
              ...
              <div key={log.id} className={`flex items-center gap-3 py-2.5 px-3 rounded-lg ${log.granted ? "hover:bg-stone-50" : "bg-rose-50 hover:bg-rose-100"}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-stone-500">
                    {log.door}{!log.granted && log.reason && <span className="text-rose-700"> · {log.reason}</span>}
                  </div>
                </div>
                <div className="text-xs font-mono text-stone-500 text-right">
                  <div>{log.time}</div>
                  <div className="text-stone-400">{new Date(log.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</div>
                </div>
              </div>
            ))}
          </div>
...
export function MessagesView({ messages, setMessages, templates, setTemplates, members }) {
  const [tab, setTab] = useState("messages"); // messages | templates | compose
      ...
      <div className="flex gap-1 bg-stone-100 p-1 rounded-lg w-fit">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-4 py-1.5 text-xs font-medium rounded-md transition ${tab === t.id ? "bg-white shadow-sm" : "text-stone-600"}`}>
            {t.label}
          </button>
        ))}
      </div>
    ...
    <div className="space-y-3">
      <div className="bg-stone-100 border border-stone-200 rounded-xl p-4">
        <p className="text-sm text-stone-700">
          Automated messages send based on triggers. Use <span className="font-mono text-xs bg-white px-1.5 py-0.5 rounded">{`{{name}}`}</span>, <span className="font-mono text-xs bg-white px-1.5 py-0.5 rounded">{`{{plan}}`}</span> etc. to personalize.
        </p>
      </div>
      {templates.map((t) => (
        <div key={t.id} className="bg-white rounded-xl border border-stone-200 p-5">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className="text-[10px] font-mono tracking-wider uppercase bg-stone-100 text-stone-700 px-2 py-0.5 rounded-full">{t.channel}</span>
              </div>
          ...
          <div className="text-sm text-stone-700 bg-stone-50 p-3 rounded-lg font-mono text-xs">{t.body}</div>
        </div>
      ))}
    </div>
  );
...
function ComposeView({ messages, setMessages, members, onSent }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-5 sm:p-6 max-w-2xl">
      <h3 className="font-display text-xl font-semibold mb-5">New broadcast</h3>

      <div className="space-y-4">
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Audience</label>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(audienceLabel).map(([key, label]) => (
              <button key={key} onClick={() => setAudience(key)}
                className={`py-2.5 px-3 rounded-lg text-sm font-medium border text-left transition ${audience === key ? "bg-stone-900 text-white border-stone-900" : "bg-white border-stone-200 hover:border-stone-400"}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Channel</label>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setChannel("SMS")} className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium border transition ${channel === "SMS" ? "bg-stone-900 text-white border-stone-900" : "bg-white border-stone-200"}`}>
              <MessageSquare className="w-4 h-4" /> SMS
            </button>
            <button onClick={() => setChannel("Email")} className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium border transition ${channel === "Email" ? "bg-stone-900 text-white border-stone-900" : "bg-white border-stone-200"}`}>
        ...
        <button onClick={send} className="w-full py-3 bg-stone-900 text-white rounded-lg text-sm font-semibold hover:bg-stone-800 flex items-center justify-center gap-2">

    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex gap-3">
          <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
            <div className="font-display text-3xl font-semibold">{upcomingCount}</div>
            <div className="text-xs font-mono text-stone-500 tracking-wider uppercase mt-0.5">Upcoming shifts</div>
          </div>
          <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
            <div className="font-display text-3xl font-semibold">{staffCount}</div>
      ...
      <div className="space-y-4">
        {days.map((date) => {
          ...
          return (
            <div key={date} className={`bg-white rounded-xl border p-5 ${isToday ? "border-red-200" : "border-stone-200"}`}>
              <div className="flex items-center gap-2 mb-4">
                <div className="text-xs font-mono tracking-widest text-stone-500 uppercase">{label}</div>
                {isToday && <span className="text-[10px] bg-red-500 text-white px-2 py-0.5 rounded-full font-mono font-semibold">TODAY</span>}
              </div>
              {dayShifts.length === 0 ? (
                <p className="text-sm text-stone-400 text-center py-4">No shifts scheduled</p>
              ) : (
                <div className="space-y-2">
                  {dayShifts.map((sh) => (
                    <div key={sh.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-stone-200 to-stone-300 flex items-center justify-center font-semibold text-stone-700 shrink-0 text-sm">
                        {sh.staffName[0]}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-sm">{sh.staffName}</div>
                        <div className="text-xs text-stone-500"><span className="capitalize">{sh.role}</span> · {sh.location}</div>
                      </div>
                      <div className="text-xs font-mono text-stone-700 font-semibold shrink-0">{sh.startTime} – {sh.endTime}</div>
                      {canEdit && (
                        <div className="flex gap-0.5 shrink-0">
                          <button onClick={() => { setEditShift(sh); setShowForm(true); }} className="p-1.5 text-stone-400 hover:text-stone-700 rounded transition">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => deleteShift(sh.id)} className="p-1.5 text-stone-400 hover:text-rose-600 rounded transition">
...
function ShiftForm({ shift, onSave, onClose }) {
  ...
  return (
    <Modal title={shift ? "Edit Shift" : "Add Shift"} onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Staff member</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Start</label>
            <input type="time" value={form.startTime} onChange={(e) => set("startTime", e.target.value)}
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900" />
          </div>
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">End</label>
            <input type="time" value={form.endTime} onChange={(e) => set("endTime", e.target.value)}
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900" />
          </div>
        </div>
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Location</label>
          <select value={form.location} onChange={(e) => set("location", e.target.value)}
            className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900">
            {["Main Floor", "Reception", "Cardio Zone", "Weight Room", "Studio"].map((l) => <option key={l} value={l}>{l}</option>)}
          </select>
        </div>
        <button onClick={() => onSave(form)} className="w-full py-3 bg-red-500 text-white rounded-lg text-sm font-semibold hover:bg-red-600 transition">
...
// ======================================================================
const STATUS = {
  operational: { label: "Operational", badge: "bg-red-100 text-red-800 border-red-200", icon: "text-red-600", bg: "bg-red-100" },
  maintenance: { label: "In Maintenance", badge: "bg-amber-100 text-amber-800 border-amber-200", icon: "text-amber-600", bg: "bg-amber-100" },
  "out-of-service": { label: "Out of Service", badge: "bg-rose-100 text-rose-800 border-rose-200", icon: "text-rose-600", bg: "bg-rose-100" },
};
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-stone-200 p-4 text-center">
          <div className="font-display text-3xl font-semibold text-red-600">{operational}</div>
          <div className="text-xs font-mono text-stone-500 tracking-wider uppercase mt-0.5">Operational</div>
        </div>
        <div className={`rounded-xl border p-4 text-center ${maintenance > 0 ? "bg-amber-50 border-amber-200" : "bg-white border-stone-200"}`}>
          <div className={`font-display text-3xl font-semibold ${maintenance > 0 ? "text-amber-700" : "text-stone-600"}`}>{maintenance}</div>
          <div className="text-xs font-mono text-stone-500 tracking-wider uppercase mt-0.5">Maintenance</div>
        </div>
        <div className={`rounded-xl border p-4 text-center ${outOfService > 0 ? "bg-rose-50 border-rose-200" : "bg-white border-stone-200"}`}>
          <div className={`font-display text-3xl font-semibold ${outOfService > 0 ? "text-rose-700" : "text-stone-600"}`}>{outOfService}</div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-display text-xl font-semibold">Equipment ({equipment.length} items)</h3>
        {canEdit && (
        ...
        <div key={cat}>
          ...
          <div className="space-y-2">
            {items.map((eq) => {
              ...
              return (
                <div key={eq.id} className={`bg-white rounded-xl border p-4 ${eq.status !== "operational" ? "border-amber-200" : "border-stone-200"}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {overdue && <span className="text-[10px] bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full font-mono">Service overdue</span>}
                      </div>
                      {eq.notes && <div className="text-xs text-stone-500 mt-0.5">{eq.notes}</div>}
                      <div className="text-[10px] font-mono text-stone-400 mt-0.5">Next service: {eq.nextService || "—"}</div>
                    </div>
                    {canEdit && (
                      <div className="flex items-center gap-1 shrink-0">
                        <select value={eq.status} onChange={(e) => setEquipment((es) => es.map((x) => x.id === eq.id ? { ...x, status: e.target.value } : x))}
                          className="text-xs border border-stone-200 rounded-lg px-2 py-1.5 bg-stone-50 focus:outline-none hidden sm:block">
                          <option value="operational">Operational</option>
                          <option value="maintenance">Maintenance</option>
                          <option value="out-of-service">Out of service</option>
                        </select>
                        <button onClick={() => { setEditItem(eq); setShowForm(true); }} className="p-1.5 text-stone-400 hover:text-stone-700 rounded transition">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    )}
...
function EquipmentForm({ item, onSave, onClose }) {
  ...
  return (
    <Modal title={item ? "Edit Equipment" : "Add Equipment"} onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Name</label>
          <input type="text" value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. Treadmill #4"
            className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Category</label>
          ...
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Status</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Last service</label>
            <input type="date" value={form.lastService} onChange={(e) => set("lastService", e.target.value)}
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900" />
          </div>
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Next service</label>
        ...
        <button onClick={() => { if (!form.name) return; onSave(form); }} className="w-full py-3 bg-red-500 text-white rounded-lg text-sm font-semibold hover:bg-red-600 transition">
...
const CAT_COLORS = {
  ...
  "Staff Wages": "bg-violet-100 text-violet-800",
  Equipment: "bg-amber-100 text-amber-800",
  Marketing: "bg-pink-100 text-pink-800",
  Supplies: "bg-stone-100 text-stone-700",
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {canEdit && (
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-2 px-4 py-2.5 bg-red-500 text-white rounded-lg text-sm font-semibold hover:bg-red-600 transition ml-auto">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          ...
          <div className="font-display text-2xl font-semibold">₦{(revenue / 1000).toFixed(0)}K</div>
        </div>
        <div className={`rounded-xl border p-5 ${totalExpenses > 0 ? "bg-rose-50 border-rose-200" : "bg-white border-stone-200"}`}>
          <div className="flex items-center gap-2 mb-2">
            <TrendingDown className="w-4 h-4 text-rose-600" />
            <div className="text-xs font-mono text-stone-500 tracking-wider uppercase">Expenses</div>
          </div>
          <div className="font-display text-2xl font-semibold text-rose-700">₦{(totalExpenses / 1000).toFixed(0)}K</div>
        </div>
        <div className={`rounded-xl border p-5 ${profit >= 0 ? "bg-red-50 border-red-200" : "bg-rose-50 border-rose-200"}`}>
          <div className="flex items-center gap-2 mb-2">
            <DollarSign className={`w-4 h-4 ${profit >= 0 ? "text-red-600" : "text-rose-600"}`} />
            <div className="text-xs font-mono text-stone-500 tracking-wider uppercase">Net P&L</div>
          </div>
          <div className={`font-display text-2xl font-semibold ${profit >= 0 ? "text-red-700" : "text-rose-700"}`}>
      ...
      {Object.keys(byCategory).length > 0 && (
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          <h3 className="font-display text-lg font-semibold mb-4">Breakdown by category</h3>
              ...
              return (
                <div key={cat}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="font-mono font-semibold">₦{amt.toLocaleString()} <span className="text-stone-400 font-normal text-xs">({pct}%)</span></span>
                  </div>
      ...
      {/* Expense list */}
      <div className="bg-white rounded-xl border border-stone-200 p-5">
        <h3 className="font-display text-lg font-semibold mb-4">Expense log</h3>
        {monthExpenses.length === 0 ? (
          ...
          <div className="space-y-2">
            {[...monthExpenses].sort((a, b) => new Date(b.date) - new Date(a.date)).map((ex) => (
              <div key={ex.id} className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full ${CAT_COLORS[ex.category]}`}>{ex.category}</span>
                    {ex.recurring && <span className="text-[10px] bg-stone-200 text-stone-600 px-2 py-0.5 rounded-full font-mono">Recurring</span>}
                  </div>
                  <div className="text-sm font-medium">{ex.description}</div>
                  <div className="text-[10px] font-mono text-stone-400">{new Date(ex.date + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}</div>
                </div>
                <div className="font-mono font-semibold text-rose-700 shrink-0">₦{ex.amount.toLocaleString()}</div>
                {canEdit && (
                  <button onClick={() => setExpenses((es) => es.filter((e) => e.id !== ex.id))} className="p-1.5 text-stone-300 hover:text-rose-600 transition shrink-0">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>
  ...
  return (
    <Modal title="Add Expense" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Amount (₦)</label>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-sm text-stone-700">Recurring monthly expense</span>
        </label>
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {VID_CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => setFilter(cat)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === cat ? "bg-red-500 text-white" : "bg-white border border-stone-200 text-stone-700 hover:border-stone-400"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((v) => (
            <div key={v.id} className="bg-white rounded-xl border border-stone-200 overflow-hidden hover:border-stone-400 transition group cursor-pointer" onClick={() => setPlaying(v)}>
              <div className="h-36 bg-gradient-to-br from-stone-800 to-stone-900 flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition bg-black/30 rounded-t-xl">
                  <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
                    <Play className="w-5 h-5 text-white ml-0.5" />
                  </div>
                </div>
                <div className="absolute bottom-2 right-2 bg-black/60 rounded px-1.5 py-0.5 text-[10px] font-mono text-white">{v.duration}</div>
                <div className="absolute top-2 left-2 bg-red-500/90 rounded-md px-2 py-0.5 text-[10px] font-mono text-white uppercase tracking-wider">{v.category}</div>
              </div>
              <div className="p-4">
                <div className="font-semibold text-sm leading-tight mb-1">{v.title}</div>
      ...
      {playing && (
        <Modal title={playing.title} subtitle={`${playing.duration} · ${playing.category} · ${playing.trainerName}`} onClose={() => setPlaying(null)}>
          <div className="space-y-4">
            <div className="h-52 bg-gradient-to-br from-stone-800 to-stone-900 rounded-xl flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-stone-600 leading-relaxed">{playing.description}</p>
...
function VideoUploadForm({ user, onSave, onClose }) {
  ...
  return (
    <Modal title="Upload Video" onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Title</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Duration</label>
            <input type="text" value={form.duration} onChange={(e) => set("duration", e.target.value)} placeholder="e.g. 15:30"
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-900" />
          </div>
        </div>
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Thumbnail emoji</label>
          <div className="flex gap-2 flex-wrap">
            {EMOJIS.map((e) => (
              <button key={e} onClick={() => set("emoji", e)}
                className={`w-10 h-10 text-2xl rounded-lg border transition ${form.emoji === e ? "border-stone-900 bg-stone-100 scale-110" : "border-stone-200 hover:border-stone-400"}`}>
                {e}
              </button>
            ))}
          </div>

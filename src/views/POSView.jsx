      ...
      <div className="flex gap-1 bg-stone-100 p-1 rounded-lg w-fit">
        {[
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`px-4 py-1.5 text-xs font-medium rounded-md transition ${activeTab === t.id ? "bg-white shadow-sm" : "text-stone-600"}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-2.5 bg-white border border-stone-200 rounded-lg text-sm">
              ...
              {filtered.map((p) => {
                ...
                return (
                  <button key={p.id} onClick={() => addToCart(p)} disabled={oos}
                    className="bg-white border border-stone-200 rounded-xl p-4 text-left hover:border-stone-900 transition disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div className="flex items-end justify-between">
                      <div className="font-display text-lg font-semibold">₦{p.price.toLocaleString()}</div>
                      {p.category !== "Services" && (
                        <div className={`text-[10px] font-mono ${oos ? "text-rose-700" : low ? "text-amber-700" : "text-stone-500"}`}>
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-5 h-fit sticky top-24">
            <div className="flex items-center gap-2 mb-4">
              ...
              {cart.length > 0 && <span className="text-xs font-mono bg-stone-100 px-2 py-0.5 rounded-full">{cart.length}</span>}
            </div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                <div className="space-y-2 mb-4 max-h-72 overflow-y-auto">
                  {cart.map((i) => (
                    <div key={i.productId} className="flex items-start gap-2 py-2 border-b border-stone-100 last:border-0">
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium truncate">{i.name}</div>
                        <div className="text-xs text-stone-500 font-mono">₦{i.price.toLocaleString()}</div>
                      </div>
                      <div className="flex items-center gap-1">
                        <button onClick={() => updateQty(i.productId, -1)} className="w-6 h-6 rounded bg-stone-100 hover:bg-stone-200 flex items-center justify-center"><Minus className="w-3 h-3" /></button>
                        <span className="w-6 text-center text-xs font-mono">{i.qty}</span>
                        <button onClick={() => updateQty(i.productId, 1)} className="w-6 h-6 rounded bg-stone-100 hover:bg-stone-200 flex items-center justify-center"><Plus className="w-3 h-3" /></button>
                        <button onClick={() => removeFromCart(i.productId)} className="w-6 h-6 rounded hover:bg-rose-100 flex items-center justify-center"><X className="w-3 h-3 text-rose-600" /></button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="border-t border-stone-200 pt-3 mb-4">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="font-display text-2xl font-semibold">₦{cartTotal.toLocaleString()}</span>
                  </div>
                </div>

                <button onClick={() => setShowCheckout(true)} className="w-full py-3 bg-stone-900 text-white rounded-lg text-sm font-semibold hover:bg-stone-800">
                  Checkout
                </button>
              </>
            )}
  ...
  return (
    <Modal title="Checkout" subtitle={`${cart.length} item${cart.length !== 1 ? "s" : ""} · ₦${cartTotal.toLocaleString()}`} onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Link to member (optional)</label>
          {selectedMember ? (
            <div className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg">
              <div className="w-8 h-8 rounded-full bg-stone-200 flex items-center justify-center font-semibold text-sm">{selectedMember.name[0]}</div>
              <div className="flex-1 text-sm font-medium">{selectedMember.name}</div>
              <button onClick={() => { setMemberId(null); setMemberSearch(""); }} className="text-xs text-stone-500 underline">Remove</button>
            </div>
          ) : (
            <>
              ...
              {matchingMembers.length > 0 && (
                <div className="mt-2 border border-stone-200 rounded-lg overflow-hidden">
                  {matchingMembers.map((m) => (
                    <button key={m.id} onClick={() => { setMemberId(m.id); setMemberSearch(""); }} className="w-full flex items-center gap-3 p-2 hover:bg-stone-50 text-left">
                      <div className="w-6 h-6 rounded-full bg-stone-200 flex items-center justify-center text-xs font-semibold">{m.name[0]}</div>
        ...
        <div>
          <label className="text-xs font-mono tracking-wider text-stone-500 uppercase mb-1.5 block">Payment method</label>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => onComplete("Cash")} className="flex items-center justify-center gap-2 py-3 border border-stone-200 rounded-lg hover:border-stone-900 text-sm font-medium">
              <DollarSign className="w-4 h-4" /> Cash
            </button>
            <button onClick={() => onComplete("Card")} className="flex items-center justify-center gap-2 py-3 border border-stone-200 rounded-lg hover:border-stone-900 text-sm font-medium">
              <CreditCard className="w-4 h-4" /> Card
            </button>
            <button onClick={() => onComplete("Transfer")} className="flex items-center justify-center gap-2 py-3 border border-stone-200 rounded-lg hover:border-stone-900 text-sm font-medium">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 py-3 border border-stone-200 rounded-lg hover:border-stone-900 text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed">
            ...
            {sorted.map((p) => {
              ...
              return (
                <tr key={p.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50">
                  ...
                  <td className="px-6 py-4 text-xs font-mono text-stone-600">{p.sku}</td>
                  <td className="px-6 py-4 text-sm font-mono">₦{p.price.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    {p.category === "Services" ? (
                      <span className="text-xs text-stone-500">—</span>
                    ) : (
                      <span className={`text-sm font-mono font-semibold ${low ? "text-rose-700" : "text-stone-900"}`}>
                        {p.stock}{low && <span className="ml-2 text-[10px] tracking-wider uppercase bg-rose-100 text-rose-800 px-1.5 py-0.5 rounded-full">Low</span>}
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    {p.category !== "Services" && (
                      <div className="inline-flex items-center gap-1">
                        <button onClick={() => adjustStock(p.id, -1)} className="w-7 h-7 rounded bg-stone-100 hover:bg-stone-200 flex items-center justify-center"><Minus className="w-3 h-3" /></button>
                        <button onClick={() => adjustStock(p.id, 1)} className="w-7 h-7 rounded bg-stone-100 hover:bg-stone-200 flex items-center justify-center"><Plus className="w-3 h-3" /></button>
                        <button onClick={() => adjustStock(p.id, 10)} className="px-2 h-7 rounded bg-stone-100 hover:bg-stone-200 text-[10px] font-mono">+10</button>
                      </div>
                    )}
                  </td>
                </tr>
          ...
          <tbody>
            {sales.map((s) => (
              <tr key={s.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50">
                ...
                <td className="px-6 py-4 text-sm font-medium">{s.memberName}</td>
                <td className="px-6 py-4">
                  <div className="text-xs text-stone-600">
                    {s.items.map((i) => `${i.qty}× ${i.name}`).join(", ")}
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-stone-600">{s.method}</td>
                <td className="px-6 py-4 text-right font-mono font-semibold">₦{s.total.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
